...
override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if touches.count == 1 {
            if let touch = touches.first
            {
                ...
                if hitPresentationLayer?.name == "Sree"
                {
                    if let subLayers = hitPresentationLayer?.sublayers
                    {
                        ...
                        for subL in subLayers
                        {
                            ...
                            let b = shapeL.path?.contains(touchLocation) ?? false
                            if b {
                                selectedVal = shapeL.name ?? ""
                                print("Selected slice name : \(shapeL.name ?? "No name found")")
                                break
                            }
                        }
                        
                        if !selectedVal.isEmpty {
                            if let custViewSubLayersN = custView.layer.sublayers {
                                
                                for subLayerN in custViewSubLayersN {
                                    let caShapeLayer = subLayerN as! CAShapeLayer
                                    
                                    if caShapeLayer.name == selectedVal {
